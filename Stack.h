...
/*Simple stack class (thread safe)
*   @author andrewmiles2
*   NOTE: Accessing parts of the stack that haven't been 
*       set yet will return garbage!
*/
...
namespace andrewmiles2{
    ...
    void Stack<T>::push(T new_item){
        //lock for exclusive update access.
        std::unique_lock<std::recursive_mutex> access_lock(access_mutex);
        if(top == size-1){
